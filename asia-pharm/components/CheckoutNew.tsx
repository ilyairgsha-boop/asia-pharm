...
export const CheckoutNew = ({ onNavigate, store }: CheckoutProps) => {
  ...
  const loadLoyaltyInfo = async () => {
    try {
      const response = await fetch(getServerUrl('/loyalty'), {
        headers: {
      ...
      if (response.ok) {
        ...
        setAvailableLoyaltyPoints(data.points || 0);
      } else {
        // If server not available, set default value
        setAvailableLoyaltyPoints(0);
      }
    } catch (error) {
      console.warn('⚠️ Server not available, loyalty points unavailable');
      setAvailableLoyaltyPoints(0);
    }
  };
  ...
  // Расчет скидки баллами (баллы можно использовать только на товары, не на доставку и пробники)
  const loyaltyDiscount = useLoyaltyPoints ? Math.min(loyaltyPointsToUse, subtotalWithoutSamples) : 0;

  const total = Math.max(0, subtotal - promoDiscount - loyaltyDiscount + shippingCost);
  ...
  const applyPromoCode = async () => {
    ...
    try {
      const response = await fetch(getServerUrl(`/promo-codes/validate?code=${promoCode}`), {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
    ...
    try {
      const response = await fetch(getServerUrl('/orders'), {
        ...
        body: JSON.stringify({
          items: cart,
          store,
          shippingInfo: { ...formData, deliveryMethod },
          subtotal,
          subtotalWithoutSamples, // Сумма без пробников для начисления баллов
          shippingCost,
      ...
      if (response.ok) {
        ...
        alert(`${t('errorPlacingOrder')}: ${error.error || ''}`);
      }
    } catch (error) {
